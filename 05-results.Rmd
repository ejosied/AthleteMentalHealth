...
MH_pivot2 <- MH_responses2[,c(1:13)] %>%
  ...
  filter(PSPORT %in% c("Dance", "Track", "Golf", "Rugby", "Soccer", "Colorguard"))

ggplot(MH_pivot2, aes(x = Assessment, stratum = Score, alluvium = CASEID,
                          y = Freq, 
                      fill = PSPORT, label = Score)) +
  geom_flow() +
  coord_flip() +
